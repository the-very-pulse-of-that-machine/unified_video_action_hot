...
class LiberoImageRunner(BaseImageRunner):
    """
    DDP / AsyncVectorEnv safe version
    - no dill
    - no run_dill_function
    """
    ...
    def __init__(
        ...
        # -------- env fn --------
        def env_fn():
            libero_env = create_env(env_meta, shape_meta, enable_render=True)
            libero_env.env.hard_reset = False
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    RobomimicImageWrapper(
        ...
        def dummy_env_fn():
            libero_env = create_env(env_meta, shape_meta, enable_render=False)
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    RobomimicImageWrapper(
                        env=libero_env,
        ...
        with h5py.File(dataset_path, "r") as f:
            for i in range(n_train):
                ...
                self.init_states.append(f[f"data/demo_{idx}/states"][0])
                self.enable_renders.append(i < n_train_vis)
                self.env_seeds.append(idx)
                self.env_prefixs.append("train/")

        for i in range(n_test):
            seed = test_start_seed + i
            self.init_states.append(None)
            self.enable_renders.append(i < n_test_vis)
    ...
    def run(self, policy: BaseImagePolicy, **kwargs):
        ...
        for i in range(n_envs):
            ...
            env.call_each(
                "set_attr",
                args_list=[("init_state", self.init_states[i])],
            )
            env.call_each(
                "set_attr",
                args_list=[("enable_render", self.enable_renders[i])],
            )
            env.call_each(
                "seed",
                args_list=[(self.env_seeds[i],)],
            )
            ...
            pbar = tqdm.tqdm(
                total=self.max_steps,
                desc=f"Eval {self.task_name} [{i+1}/{n_envs}]",
                mininterval=self.tqdm_interval_sec,
                leave=False,
            )
            ...
            while not done:
                obs_dict = dict_apply(
                    obs, lambda x: torch.from_numpy(x).to(device)
                )
            ...
            all_video_paths[i] = env.render()[0]
            all_rewards[i] = env.call("get_attr", "reward")[0]
        ...
        for i in range(n_envs):
            ...
            if all_video_paths[i] is not None:
                log_data[f"{prefix}sim_video_{seed}"] = wandb.Video(
                    all_video_paths[i]
                )
    ...
    def undo_transform_action(self, action):
        ...
        grip = action[..., [-1]]
        rot = self.rotation_transformer.inverse(rot)
        uaction = np.concatenate([pos, rot, grip], axis=-1)
